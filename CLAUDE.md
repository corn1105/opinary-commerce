...
- **Prices + buy buttons are backfilled, not generated.** Claude only emits `title/description/query/badge` — no price. The widget's "Buy now" button **only renders when a product has a `price`**, so prices must be filled by `scripts/backfill_prices_and_merchants.py` after generation. Prices are *category-estimated* (hardcoded tier ranges keyed by **poll id** in `POLL_ID_TO_CATEGORY`), not real Amazon prices — there is no PA-API account. Images + ASINs come from `scripts/backfill_real_images.py` (best-effort Amazon CDN scrape; Amazon frequently bot-blocks, so it's run-to-converge and ASIN coverage is partial → buy links fall back to `/s?k=<query>`). Secondary "compare" merchant is locale-aware (Best Buy for `en`, Otto for `de`) via `SECONDARY_MERCHANT` in `public.py`. **Restore pipeline:** `fill_missing_recs.py --locale {en,de}` → `backfill_real_images.py` → `backfill_prices_and_merchants.py`.
...
- **Bridge regeneration regenerates products too.** They're a single Claude call. Editing just the bridge uses the PATCH endpoint and leaves products alone. **Regeneration (admin "Regenerate" or a vote-time cache miss) drops `price`/`amazon_asin`/`image_url`** — the fresh Claude products have none — so the buy button disappears for that option until the backfill pipeline is re-run on it.
